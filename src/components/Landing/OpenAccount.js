...
<button onClick = {() =>handleClick("borrower")} className={` ${toggleButtonBorrower ? "" :"opacity-60"} capitalize text-white  bg-[#F5BD74] text-[20px] ml-[10px] font-medium w-[161px] rounded-[50px] py-[15px]`}>borrower</button>
    ...
    <div className='mt-[75px]'> <Link to = {`/${toggleButtonInvest ? 'investor' : 'borrower'}`}><button onClick = {() =>handleClick("investor")} className={` bg-[#36C3D6] capitalize text-white  text-[20px] font-medium rounded-[10px] py-[16px] px-[59px]  mb-[48px]`}>{toggleButtonInvest ? 'become an investor':'become a borrower'}</button></Link></div>
   </div>
  );
}
